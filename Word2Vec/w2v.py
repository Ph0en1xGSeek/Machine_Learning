...
def build_dataset(words):
    ...
    #data:rank of each position in words
    count = [['UNK', -1]]
...
valid_size = 16 #number of valid words
valid_window = 100 #number of the most common words used for valid. from 100 select 16
...
with graph.as_default():
    ...
    normalized_embeddings = embeddings / norm #norm the vec to 1
    valid_embeddings = tf.nn.embedding_lookup(normalized_embeddings, valid_dataset)
    similarity = tf.matmul(valid_embeddings, normalized_embeddings, transpose_b=True)#16个验证的词的向量和其他向量的余弦相似度
...
with tf.Session(graph = graph) as session:
    ...
    def plot_with_labels(low_dim_embs, labels, filenames='tsne.png'):

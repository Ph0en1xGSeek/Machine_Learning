...
def x2p(X = np.array([]), tol = 1e-5, perplexity = 30.0, nnrange = 3.0):
...
def tsne(X = np.array([]), no_dims = 2, initial_dims = 50, perplexity = 30.0, use_pca = True, nnrange = 3.0):
	...
	# Compute P-values
	P = x2p(X, 1e-5, perplexity, nnrange=nnrange);
	#用Pij+Pji解决对称性
	P = P + np.transpose(P);
	P = P / np.sum(P);
...
if __name__ == "__main__":
	...
	Y = tsne(X, 2, 50, 100.0, use_pca=True, nnrange=9.0);# use pca to reduce the dimension from 784 to 50. Then use t-SNE from 50 to 2
	Plot.figure(20)
